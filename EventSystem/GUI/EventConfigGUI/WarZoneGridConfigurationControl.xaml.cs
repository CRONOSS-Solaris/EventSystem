    ...
    public partial class WarZoneGridConfigurationControl : UserControl
    {
        ...
        private void AddDay_Click(object sender, RoutedEventArgs e)
        {
            if (int.TryParse(NewDayTextBox.Text, out int newDay))
            {
                ...
                {
                    if (!Plugin.Config.WarZoneGridSettings.ActiveDaysOfMonth.Contains(newDay))
                    {
                        Plugin.Config.WarZoneGridSettings.ActiveDaysOfMonth.Add(newDay);
        ...
        private void RemoveSelectedDay_Click(object sender, RoutedEventArgs e)
        {
            if (int.TryParse(NewDayTextBox.Text, out int dayToRemove))
            {
                ...
                {
                    if (Plugin.Config.WarZoneGridSettings.ActiveDaysOfMonth.Contains(dayToRemove))
                    {
                        Plugin.Config.WarZoneGridSettings.ActiveDaysOfMonth.Remove(dayToRemove);
                        // Aktualizacja ItemsControl
                        UpdateDaysOfMonthItemsControl();
                        NewDayTextBox.Clear();
                    }
