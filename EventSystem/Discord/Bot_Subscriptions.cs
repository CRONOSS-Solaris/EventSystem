...
using static EventSystem.EventSystemMain;

namespace EventSystem.Discord
{
    public sealed class Bot_Subscriptions
    {
        private Bot _bot => DiscordBot;
        private List<DiscordGuild> m_guilds = new List<DiscordGuild>();
        private Logger Log = LogManager.GetLogger("EventSystemMain/DiscordBotMain/Bot_Subscriptions");
        ...
        {
            await LoggerHelper.InfoAsync(Log, "Guild Download Completed");
            // Make sure we have all Guild(s) data!
            foreach (KeyValuePair<ulong, DiscordGuild> discordGuild in _bot.Client.Guilds)
            {
            ...
            _bot.ServerData.guild = m_guilds[0];
            await LoggerHelper.InfoAsync(Log, "Guild Data Retrieved for " + m_guilds[0].Name + " (" + m_guilds[0].Id + ")");

            // Get all members
            m_members = await m_guilds[0].GetAllMembersAsync();
            await LoggerHelper.InfoAsync(Log, $"{m_members.Count} members retrieved.");
            _bot.ServerData.roles = m_guilds[0].Roles;
            ...
            //        catch (Exception e)
            //        {
            //            await LoggerHelper.ErrorAsync("Managed Role ID provided is invalid: " + e);
            //        }
            //    }
            //}

            // Set Bot Name, Status
            if (Instance.DiscordBotConfig.DiscordBotName != DiscordBot.BotUser.Username)
            {
                await DiscordBot.Client.UpdateCurrentUserAsync(Instance.DiscordBotConfig.DiscordBotName);
                await DiscordBot.ServerData.guild.CurrentMember.ModifyAsync(mdl => mdl.Nickname = Instance.DiscordBotConfig.DiscordBotName);
            }
            ...
            if (Instance.WorldOnline)
                ...
                await DiscordBot.Client.UpdateStatusAsync(new DiscordActivity("Waiting for the server", ActivityType.Watching), UserStatus.DoNotDisturb);
        }
        ...
        {
            Instance.DiscordBotConfig.BotStatus = "Connected";
            Log.Info("Connected.");
